 ...
 * Login subscriber via captive portal.
 * Checks MAC blacklist before authenticating.
 */
export async function POST(request: NextRequest) {
  try {
    ...
    const { username, password, macAddress, locationId } = body;
    ...
    if (subscriber.status === "blocked") {
      return NextResponse.json({ error: "Akun Anda diblokir. Hubungi admin." }, { status: 403 });
    }
    ...
    const waktuHabis =
      subscriber.expiredAt !== null && new Date() > subscriber.expiredAt;

    if (quotaHabis || waktuHabis || subscriber.status === "expired") {
      return NextResponse.json({ error: "Kuota atau masa berlaku habis", code: "EXPIRED" }, { status: 403 });
    }
