...
export async function POST(
  ...
  try {
    ...
    // Reset quota in app database
    await db.subscriber.update({
      where: { id: subId },
      data:  { quotaUsedMb: BigInt(0) },
    });

    // Reset quota-related radcheck attributes
    // Remove any Max-All-Session or similar quota enforcement entries
    await db.$executeRaw`
      DELETE FROM radcheck
      WHERE username = ${sub.username}
        AND attribute IN ('Max-All-Session', 'Max-Daily-Session', 'Acct-Interim-Interval')
    `;
    ...
    await db.adminLog.create({
      data: {
        adminId:     admin.id,
        action:      "RESET_QUOTA",
        description: `Reset kuota subscriber "${sub.username}" (ID ${subId})`,
    ...
    console.error("[users/reset-quota]", err);
